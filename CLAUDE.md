...
- **Swift Testing runs separate suites in parallel.** `.serialized` only orders tests *within* one suite, so anything reached through a process-wide global is shared with whatever else is running. Two globals matter here: `KeychainService.backend` and `Preferences.twitchReauthNeeded`. A suite that installs its own Keychain backend binds `KeychainService.backendBox` (a `@TaskLocal`) via the `.isolatedKeychainBackend` trait, which makes that backend unreachable from any other task tree; `KeychainServiceTests` uses it, and two tests in that suite pin the property so the trait cannot silently stop binding. Anything else that touches either global wraps the work in `KeychainBackendTestIsolation.withIsolatedCredentialState { }`. A `@MainActor` XCTest suite must take that lock with `acquireAsync()` from `setUp() async throws` — a blocking `acquire()` on the main thread deadlocks whenever the current holder is a `@MainActor` test suspended mid-`await`. Guarding only the suites that *swap* the backend is not enough: an ordinary reader racing a swap was itself a flake source. **A `@TaskLocal` does not reach code that leaves the task tree** — a test that drives `KeychainService` from a `DispatchQueue` block or a `Task.detached` sees the process-wide backend instead, and must rebind with `KeychainService.$backendBox.withValue(box) { }` inside the block (this works off-task; `testConcurrentAccess` does it).
- **Never touch `UserDefaults.standard` in a test.** The test bundle is hosted, so `.standard` inside a test process is literally the dev app's live preference domain (`com.mrdemonwolf.wolfwave.dev`). Tests writing there edited the developer's real settings; one such write (`voteSkipWindowSeconds = 1`, not a valid picker tag) crashed the Song Requests pane inside SwiftUI on the next launch. Read and write `DefaultsStore.store` (production routes through the same seam, so a test write is visible to the code under test), or hand a SUT a throwaway `UserDefaults(suiteName:)`. Enforced by `DefaultsStoreTests` and the blocking `user_defaults_standard` SwiftLint rule.
