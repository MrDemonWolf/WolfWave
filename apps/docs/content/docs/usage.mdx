...
WolfWave serves the widget HTML and now-playing data from your Mac; no hosted
overlay service is involved. Fresh album artwork and online integrations still require internet access.

<Steps>

<Step>Open **Settings → Stream Widgets**.</Step>
<Step>Click **Enable Widget Server**.</Step>
<Step>Copy the **Widget Link**. On the same Mac, WolfWave injects the WebSocket credential into the served HTML, so it does not appear in the localhost URL.</Step>
...
|---|---:|---:|
| Horizontal *(default)* | 500 × 100 | **532 × 132** |
