...
Announcements have two requirements: the account you signed in with must be a moderator of the channel (type `/mod <account>` in chat if it's a separate bot account), and your Twitch sign-in must include the announcements permission. Sign-ins from before this feature exist without it; when a command uses Announcement, WolfWave asks you to reconnect once to grant it. If Twitch refuses an announcement for any reason, the command falls back to a reply so nobody is left unanswered, and the Custom Commands card tells you why.
