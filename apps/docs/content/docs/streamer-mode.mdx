---
title: Streamer Mode
description: Streamer Mode masks WolfWave's sensitive values like your Twitch channel, overlay URLs, and WebSocket tokens, so the app is safe to show on camera. It only changes the screen.
ogTitle: WolfWave Streamer Mode
ogDescription: One toggle masks your channel name, overlay and widget URLs, and WebSocket tokens across the UI. Safe to show on stream. Your feeds keep working.
ogEyebrow: Guide
ogChips:
  - Camera safe
  - Masks URLs & token
