...
- **Role-authenticated**: Binds to all interfaces on `:8765`; `wolfwave.overlay.<hex>` clients may receive state from LAN or loopback, while the separate `wolfwave.control.<hex>` role is accepted only from loopback and alone authorizes commands — see [Security](/docs/security#websocket-server-security).
