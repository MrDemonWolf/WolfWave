...
│   └── WebSocket/                 # WebSocketServerService (+ StreamDeck control commands) + WidgetHTTPService (token-gated)
...
- **JSON Broadcasting**: Sends `welcome`, `now_playing`, `progress`, `playback_state`, `widget_config`, and (for the Stream Deck plugin) `queue_state` / `health` messages.
...
not rebuild it); CI rebuilds it before every `xcodebuild` invocation and fails
the PR on any drift between the `apps/widget/` sources and the committed output. See the
