...
│   └── WebSocket/                 # Token-gated WebSocketServerService (+ Stream Deck commands) + static WidgetHTTPService
...
- **LAN-reachable, token-gated**: Binds to all interfaces on `:8765` so a second-PC OBS or phone on the same network can connect. Every WebSocket connection, loopback or LAN, must present `Sec-WebSocket-Protocol: wolfwave.token.<hex>` matching the per-install token in the macOS Keychain. See [Security](/docs/security#websocket-server-security).
...
- **LAN-reachable**: Binds to all interfaces on `:8766` so remote browsers can pull the widget HTML. The static HTTP response is not authenticated.
- **Credential bootstrap**: Responses inject the token only for a loopback TCP peer plus a literal local `Host`, and mark token-bearing HTML `no-store`. LAN URLs carry `?token=…`; the browser script forwards it in the authenticated WebSocket handshake.
