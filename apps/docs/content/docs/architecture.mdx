...
- **Bidirectional control channel**: Parses inbound `command` frames from an authenticated client (the Stream Deck plugin) via an injected `onCommand` handler and replies with an `ack`. Envelope decode is pure (`StreamDeckControl.parse` in `Services/WebSocket/StreamDeckCommand.swift`); the command router lives in `AppDelegate+StreamDeck`. Full protocol on the [Stream Deck](/docs/stream-deck) page.
...
- **LAN-reachable, token-gated**: Binds to all interfaces on `:8765` so a second-PC OBS or phone on the same network can connect; every connection must present the per-install auth token — see [Security](/docs/security#websocket-server-security).
...
- **LAN-reachable**: Binds to all interfaces on `:8766` so remote browsers can pull the widget HTML. The static HTTP shell is not authenticated; the token bootstrap rules are covered in [Security](/docs/security#websocket-server-security).
...
- **`src/wolfwave/protocol.ts`** — pure wire layer and the TypeScript mirror of
  ...
  pins the full v1 set so the two can't drift silently.
