...
- **Custom bot commands** (triggers, replies, aliases, permissions, cooldowns)
...
System-backed preferences take effect during import too. WolfWave updates its appearance and update settings immediately and asks macOS to apply Launch at Login. If macOS rejects that login-item change, WolfWave keeps the real system setting. If macOS accepts it but needs your approval, the import still counts it as restored and points you to **System Settings → General → Login Items**.

Song Requests can only turn on after setup is complete on that Mac. Until then, import leaves the master switch off, excludes it from the review count, and tells you why. Finish setup in **Settings → Song Requests**, then turn it on.
...
- **Hand-edited values are checked before import.** Wrong value types, malformed custom commands, unknown picker choices, and values outside a control's choices are skipped. The review count includes only preferences WolfWave can restore on this Mac.
