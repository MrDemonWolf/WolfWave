---
title: Install
description: Download WolfWave free for macOS 26+ on Apple Silicon. Install the signed .dmg or run `brew install --cask mrdemonwolf/den/wolfwave`. Free Apple Music bot for Twitch.
ogTitle: Install WolfWave. Free Apple Music Bot for Twitch on Mac
ogDescription: Two-minute install. DMG or Homebrew. macOS 26+ on Apple Silicon. Signed and notarized. Free and open source.
keywords:
  - install wolfwave
  - download wolfwave mac
  - brew install wolfwave
  - brew tap mrdemonwolf/den
  - apple music twitch chat bot mac
  - free apple music twitch bot
  - macos 26 streamer app
ogChips:
  - DMG
  - Homebrew
  - Apple Silicon
  - Free
ogEyebrow: Install in 2 minutes
---

## What you need

- **macOS 26.0+ (Tahoe).** Older versions won't run it.
- **Apple Silicon (M1 or later).** Intel Macs aren't supported.
- **The Apple Music app.** WolfWave reads only from Apple Music, which is built into macOS. Spotify and other players don't work.

## Get WolfWave

<Tabs items={["DMG (recommended)", "Homebrew"]}>

<Tab value="DMG (recommended)">

**Pick this if you're not sure.** It's the normal installer, signed and notarized, so no security warnings.

<Cards>
  <Card
    title="Download the .dmg"
    href="/download"
    description="Latest signed release direct from GitHub."
  />
</Cards>

1. **Open the `.dmg`** you just downloaded.
2. **Drag WolfWave into Applications.**
3. **Launch it** from Spotlight or Launchpad.

</Tab>

<Tab value="Homebrew">

**For developers** who want one-command installs and `brew upgrade`.

```bash
brew install --cask mrdemonwolf/den/wolfwave
```

Updates come through Homebrew. The in-app Sparkle updater is off in this mode.

```bash
brew upgrade --cask wolfwave
```

</Tab>

</Tabs>

<Callout type="info">
  WolfWave is signed and notarized by Apple. No security warnings, no right-click "Open Anyway" dance.
</Callout>

## First launch

**A setup wizard runs the first time you open WolfWave.** It's 8 short steps. Skip any optional one and finish it later from **Settings**.

<Steps>

<Step>

### Welcome

**A quick intro.** What WolfWave does and what you're about to set up.

</Step>

<Step>

### Discord *(optional)*

**Sign in to your Discord desktop app.** WolfWave talks to it locally. No bot token, no server to run.

</Step>

<Step>

### Twitch *(optional)*

**Connect your Twitch account.** You're sent to twitch.tv to approve. Your login is stored safely in your Mac's Keychain.

</Step>

<Step>

### OBS Widget *(optional)*

**Add a now-playing overlay to OBS.** Turn on the widget, copy the localhost URL, and paste it into OBS as a Browser Source. WolfWave injects the WebSocket credential into the served HTML, so the localhost URL does not expose it.

</Step>

<Step>

### Preferences

**Tweak the small stuff.** Launch at login, song-change notifications, and other handy toggles.

</Step>

<Step>

### Apple Music Access

**Click OK when macOS asks.** This is required. It's how WolfWave reads the song that's playing.

</Step>

<Step>

### Notifications (optional)

**Pick which alerts you want:** song-change banners, skip-vote started, skip-vote passed. Turn on notifications first, then choose. Skip any you don't need.

</Step>

<Step>

### Menu Bar Pointer

**Find the wolf icon.** An arrow points to it, top-right of your screen.

</Step>

<Step>

### Done

**You're set.** Click the wolf icon any time to see what's playing or open **Settings**.

</Step>

</Steps>

---

Love it? [**Sponsor WolfWave on GitHub →**](https://github.com/sponsors/nathanialhenniges)

<Cards>
  <Card title="Usage Guide" href="/docs/usage" description="Learn all the features." />
  <Card title="Bot Commands" href="/docs/bot-commands" description="Every chat command explained." />
  <Card title="Nightly Builds" href="/docs/nightly" description="Want the newest builds off main? Opt into the Nightly channel." />
</Cards>
