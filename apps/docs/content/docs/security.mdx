...
The local WebSocket server binds to all interfaces so a second-PC OBS or phone browser on the same LAN can connect. Every WebSocket connection, loopback or LAN, must present the `wolfwave.token.<hex>` `Sec-WebSocket-Protocol` token minted on first launch and stored in the macOS Keychain. See [WebSocket Server Security](#websocket-server-security) for details.
...
- Token refresh is handled for you when possible
...
- Every WebSocket connection, loopback **and** LAN, must present `Sec-WebSocket-Protocol: wolfwave.token.<hex>` on the handshake; the listener rejects everything else.
- `WidgetHTTPService` serves an unauthenticated static shell. It injects the token only when the TCP peer is loopback **and** `Host` is a literal local name/address (`localhost`, `127/8`, or `[::1]`), preventing DNS-rebinding bootstrap; token-bearing responses use `Cache-Control: no-store`.
- LAN URLs use `?token=…` as bootstrap; the browser script forwards it in the WebSocket subprotocol. The query does not authenticate the HTTP request. The HTML and HTTP response both enforce `no-referrer`, so cross-origin artwork requests cannot disclose the URL token.
...
External network traffic uses encrypted transport. Local browser-source traffic is plaintext HTTP/WS on the same Mac or LAN:
...
| `0.0.0.0:8765` | WS (plaintext LAN) | Now-playing + Stream Deck channel. Token-gated via `Sec-WebSocket-Protocol`; LAN URLs use `?token=…` only to bootstrap that header |
| `0.0.0.0:8766` | HTTP (plaintext LAN) | Unauthenticated widget shell; verified literal-local requests receive a `no-store` response with the WS credential; all widget HTML is `no-referrer` |
| Local Unix socket | IPC | Discord Rich Presence (`$TMPDIR/discord-ipc-{0..9}`) |

The WebSocket and Widget HTTP listeners bind to all interfaces so a second-PC OBS or phone browser can connect. Only WebSocket connections must present the 32-byte hex token (minted on first launch, stored in macOS Keychain, never logged in full). Because the local transport is not encrypted and the HTTP shell is public, expose these ports only on a trusted LAN.
