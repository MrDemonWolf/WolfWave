...
- **LAN-reachable**: Binds to all interfaces on port `8765`. Every WebSocket connection (loopback or LAN) must present `Sec-WebSocket-Protocol: wolfwave.token.<hex>` or it is rejected. A verified loopback TCP peer plus literal local `Host` gets a `no-store` HTML response with the token; a LAN URL carries `?token=<hex>` only as bootstrap for the WebSocket subprotocol.
...
- **LAN-reachable**: Binds to all interfaces on port `8766`. The static HTTP shell is not authenticated; WebSocket authentication happens on port `8765`.
- **Credential bootstrap**: Only verified loopback + literal-local Host responses inject the token, with `Cache-Control: no-store`. LAN URLs use `?token=<hex>`, which the browser script forwards as the WebSocket subprotocol.
...
- `design-system/tokens.json` or `design-system/scripts/generate.ts`
- `apps/widget/tailwind.config.ts`, `postcss.config.js`, or `build.ts`

### How rebuilds happen

| Trigger | Mechanism |
|---------|-----------|
| `Cmd+B` in Xcode | Bundles the committed `widget.html`; Xcode does not invoke Bun |
| CI (test + release) | Sets up Bun and rebuilds before `xcodebuild`; test CI checks artifact drift |
| Widget-only | Run `make widget` or `bun run --filter widget build` from the repo root |
| Ordered root build | Run `bun run build`; Turborepo runs token generation before the widget |

Run `bun run tokens` before a widget-only build when token definitions or the
generator changed. Committing the generated HTML keeps
native-only Xcode builds independent of the JavaScript toolchain.
