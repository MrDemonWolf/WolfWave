...
To run it alongside the now-playing card, either add `&queueTicker=1` to your
existing widget URL (both render in the same Browser Source, sized to fit
both), or add a second Browser Source pointed at the same widget URL with
`?queueTicker=1` and position it independently in your scene, the same way
chat boxes and alert widgets are typically composed in OBS.
