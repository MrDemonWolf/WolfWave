...
The same WebSocket is bidirectional: Stream Deck plugins and other local
clients send control commands (skip, hold, overlay toggle) in the reverse
direction. The command protocol lives on the
[Stream Deck page](/docs/stream-deck).
...
The widget only handles the auth token; the token itself and the WebSocket
handshake are documented in the
[security model](/docs/security#websocket-server-security). Widget-specific
behavior:

- **Loopback pages** get the token injected into the served HTML.
  `WidgetHTTPService` returns those responses with `Cache-Control: no-store`,
  so the token never sits in a browser cache.
- **LAN pages** read `?token=<hex>` from the URL and forward it when opening
  the WebSocket. Every widget document sets `no-referrer`, so cross-origin
  artwork requests can't leak that query token.
