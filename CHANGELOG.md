...
- **A blank now-playing card now says why.** If macOS hasn't granted WolfWave permission to control Music, the card shows the permission notice instead of a generic "Script error", so you know to allow it in System Settings → Privacy & Security → Automation.
...
- An Automation denial is diagnosable again. macOS 26 answers a refused Apple Event with `-1728` (`errAENoSuchObject`), not the documented `-1743`, so `AppleMusicSource.status(forBridgeError:)` fell through to its default branch and the user saw "Script error" with no hint that Automation needed granting. Confirmed on macOS 26: a client without the grant gets `osascript is not allowed assistive access. (-1728)`. Reading `-1728` as a denial is only sound for the `player state` read, an application-level property that always resolves while Music is running and after `isRunning` plus `responds(to:)` have passed. On the `currentTrack` and metadata reads `-1728` genuinely means "nothing loaded", and mapping it there would raise a permission banner every time Music sat idle. So the mapping now takes a `Constants.BridgeStage`, which also rides along in the diagnostic so a logged bridge failure names *which* Apple Event was refused. Both directions are covered by tests.
