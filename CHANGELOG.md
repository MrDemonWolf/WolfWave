...
- New `streamdeck` CI job runs `typecheck` + `bun test` for `apps/streamdeck/`. Its six test files were previously run by no workflow, so the "change one side, change the other" rule coupling `protocol.ts` to `StreamDeckCommand.swift` had no automated gate. The path filter includes the Swift file, so a Swift-only change still re-runs the TypeScript tests.
...
- `AUDIT.md` and `TODO.md` removed. The audit's 100 findings were all resolved back in 2.0.0 and it cited two files that no longer exist; the TODO was scoped to a merged PR and its premise ("nothing has ever been compiled") no longer held. Remaining open items live in Jira.
